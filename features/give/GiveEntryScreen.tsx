// GiveEntryScreen.tsx (React Native + NativeWind)
// ✅ No Button component
// ✅ Uses Pressable
...
import { ArrowLeft, Church, Calendar, Users, Heart, Sparkles, ChevronRight } from "lucide-react-native";

import { Card, CardContent } from "@/shared/components/ui";

interface GiveEntryScreenProps {
  onBack: () => void;
  onSelectTarget: (target: "church" | "event" | "program" | "group" | "other") => void;
}

export function GiveEntryScreen({ onBack, onSelectTarget }: GiveEntryScreenProps) {
      ...
      <ScrollView contentContainerClassName="pb-8">
        ...
        <View className="bg-indigo-600 px-6 py-6">
          <View className="max-w-md mx-auto">
            <Text className="text-white text-2xl font-bold mb-2">Support God&apos;s Work</Text>
            <Text className="text-indigo-100">
              Choose where you&apos;d like to give. Your generosity makes a difference in the lives of many.
            </Text>
          </View>
        </View>
        ...
        <View className="px-4 -mt-4">
                  ...
                  <Card>
                    <CardContent className="p-4">
                      <View className="flex-row items-start gap-4">
                        <View className={["p-3 rounded-2xl", target.iconBg].join(" ")}>
                          <Icon size={24} color={target.iconColor} />
                        </View>

                        <View className="flex-1">
                          <Text className="font-semibold text-gray-900 mb-1">{target.title}</Text>
                          <Text className="text-sm text-gray-600">{target.description}</Text>
                        </View>
          ...
          <View className="mt-6 p-4 bg-white rounded-2xl border border-gray-200">
            <Text className="font-semibold text-gray-900 mb-2">About Giving</Text>
          ...
          <View className="mt-4 p-4 bg-indigo-50 rounded-2xl border border-indigo-100">
            ...
            <Text className="text-xs text-indigo-700 font-semibold">2 Corinthians 9:7</Text>
          </View>
        </View>
      </ScrollView>
    </SafeAreaView>
