...
export function PrayerDetailScreen({ prayerId, onBack }: PrayerDetailScreenProps) {
      ...
      <ScrollView contentContainerClassName="p-4 pb-28">
        ...
        <View className="bg-white rounded-2xl p-6 shadow-sm mb-4">
          ...
          {!!prayer.targetClergy?.length && (
            <View className="mb-4">
              <Text className="text-sm text-gray-500 mb-2">Clergy members:</Text>
              <View className="gap-1">
                {prayer.targetClergy.map((clergy: string, index: number) => (
                  <Text key={`${clergy}-${index}`} className="text-sm text-gray-700">
                    • {clergy}
                  </Text>
                ))}
              </View>
            ...
            <View className="flex-row items-center justify-between">
              <View className="flex-row items-center gap-2">
                ...
                <Text className="text-sm text-gray-600">
                  ...
                  {prayer.responseCount === 1 ? "person is" : "people are"} praying
                </Text>
              </View>
            </View>
          </View>
