// GiveDetailScreen.tsx (React Native + NativeWind)
...
interface GiveDetailScreenProps {
  targetType: "church" | "event" | "program" | "group" | "other";
  onBack: () => void;
  onComplete?: () => void;
}
  ...
  return (
    ...
    >
      <View className="flex-row items-center gap-3">
        ...
        >
          {selected ? <View className="h-2.5 w-2.5 rounded-full bg-indigo-600" /> : null}
        </View>
        ...
        {rightTag ? (
          <View className="px-2 py-1 rounded-full bg-gray-100 border border-gray-200">
            <Text className="text-xs font-semibold text-gray-700">{rightTag}</Text>
          </View>
        ) : null}
      </View>
    </Pressable>
                ...
                return (
                  ...
                  >
                    <View className="flex-row items-center justify-between">
                      <Text className="text-gray-900 font-medium">{opt.label}</Text>
...
export function GiveDetailScreen({ targetType, onBack, onComplete }: GiveDetailScreenProps) {
    ...
    <SafeAreaView className="flex-1 bg-gray-50">
      ...
      <View className="bg-white border-b border-gray-200 px-4 py-3">
        <View className="flex-row items-center gap-3">
          ...
          <View className="flex-1">
            <Text className="font-semibold text-lg text-gray-900">Make a Donation</Text>
      ...
      <ScrollView contentContainerClassName="pb-28">
        <View className="p-4">
          ...
          {targetType === "church" ? (
            <SelectRow
              label="Select Church"
              value={churchOptions.find((x) => x.value === selectedChurch)?.label}
          ...
          <SelectRow
            ...
            value={purposeOptions.find((x) => x.value === selectedPurpose)?.label}
          ...
          {/* Amount */}
          <View className="mb-6">
            <Text className="text-sm font-semibold text-gray-900 mb-3">Donation Amount</Text>
            ...
            <View className="flex-row flex-wrap gap-2">
              {presetAmounts.map((preset) => {
                ...
                return (
                  ...
                  >
                    <Text className="text-base font-semibold text-gray-900">${preset}</Text>
                  </Pressable>
                );
              })}
            </View>
            ...
            <View className="mt-3">
              <Text className="text-xs font-semibold text-gray-700 mb-2">Custom amount</Text>
          ...
          {paymentMethod === "card" ? (
            <View className="mb-6 gap-4">
              <View>
                <Text className="text-sm font-semibold text-gray-900 mb-2">Card Number</Text>
                <TextInput
              ...
              <View className="flex-row gap-3">
                <View className="flex-1">
                  <Text className="text-sm font-semibold text-gray-900 mb-2">Expiry Date</Text>
                  <TextInput
                    placeholder="MM/YY"
                    placeholderTextColor="#9ca3af"
                    keyboardType="number-pad"
                ...
                <View className="flex-1">
                  <Text className="text-sm font-semibold text-gray-900 mb-2">CVV</Text>
                  <TextInput
                    placeholder="123"
                    placeholderTextColor="#9ca3af"
                    keyboardType="number-pad"
              ...
              <View>
                <Text className="text-sm font-semibold text-gray-900 mb-2">Name on Card</Text>
                <TextInput
            ...
            <View className="mb-6 p-4 bg-gray-50 rounded-xl border border-gray-200">
              <Text className="font-semibold text-gray-900 mb-1">Bank Transfer</Text>
          ...
          <View className="mb-6 p-4 bg-indigo-50 rounded-xl border border-indigo-100">
            <View className="flex-row gap-3">
              ...
              <View className="flex-1">
                <Text className="font-semibold text-indigo-900">Tax Receipt</Text>
                <Text className="text-sm text-indigo-800 mt-1">
                  You will receive an email receipt for your donation. Tax receipts
                  are generated automatically at year-end.
                </Text>
              </View>
            </View>
          </View>
          ...
          <View className="p-4 bg-white rounded-xl border border-gray-200">
            <Text className="font-semibold text-gray-900 mb-3">Donation Summary</Text>
            ...
            <View className="mt-3">
              <Chip label={paymentMethod === "card" ? "Card" : "Bank Transfer"} />
            </View>
          </View>
        </View>
      </ScrollView>
