...
import { ArrowLeft, Calendar, Users, MapPin, Share2, UserPlus } from "lucide-react-native";
...
export function ChurchGroupDetailScreen({ groupId, onBack }: ChurchGroupDetailScreenProps) {
  const group = useMemo(
    ...
    <View className="flex-1 bg-gray-50">
      ...
      <ScrollView contentContainerClassName="pb-28" className="-mt-6">
        ...
        <View className="bg-white rounded-t-3xl relative px-4 pt-6 pb-6">
          ...
          <Text className="text-2xl font-bold text-gray-900 mb-2">{group.name}</Text>
          ...
          <View className="flex-row gap-4 mb-6 p-4 bg-gray-50 rounded-xl">
            ...
            <View className="flex-row items-center gap-2 ml-4">
              ...
              <View>
                <Text className="text-sm font-medium text-gray-900">Weekly</Text>
          ...
          {/* Description */}
          <View className="mb-6">
            <Text className="font-semibold text-gray-900 mb-2">About This Group</Text>
            <Text className="text-sm text-gray-600 leading-6">{group.description}</Text>
          </View>
          ...
          <View className="mb-6 p-4 bg-indigo-50 rounded-xl border border-indigo-100">
            <View className="flex-row items-center gap-2 mb-2">
              ...
              <Text className="font-semibold text-gray-900">Meeting Schedule</Text>
            </View>
            <Text className="text-sm text-gray-700">{group.meetingSchedule}</Text>
            <Text className="text-xs text-gray-500 mt-1">Community Hall, Main Building</Text>
          </View>

          {/* Leaders */}
          <View className="mb-6">
            <Text className="font-semibold text-gray-900 mb-3">Group Leaders</Text>
              ...
              {group.leaders.map((leader: any, index: number) => (
                ...
                >
                  ...
                  <View className="flex-1">
                    <Text className="font-medium text-gray-900">{leader.name}</Text>
          ...
          {/* Upcoming Events */}
          <View className="mb-2">
            <Text className="font-semibold text-gray-900 mb-3">Upcoming Group Events</Text>

            <View className="gap-2">
              <View className="p-3 bg-gray-50 rounded-xl">
                <Text className="font-medium text-sm text-gray-900">Fellowship Night</Text>
                <Text className="text-xs text-gray-500">Friday, Jan 30 at 7:00 PM</Text>
              </View>

              <View className="p-3 bg-gray-50 rounded-xl">
                <Text className="font-medium text-sm text-gray-900">Community Service Day</Text>
                <Text className="text-xs text-gray-500">Saturday, Feb 6 at 9:00 AM</Text>
              </View>
            </View>
          </View>
        </View>
